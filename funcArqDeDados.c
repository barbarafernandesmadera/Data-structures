...
Registro* alocaNovoRegistro() {
    ...
    //// Inicialização de todos os campos da estrutura aqui, se necessário.
...
char* getfield(char* line, int num) {
    ...
    while ((end = strchr(start, ',')) != NULL || (end = strchr(start, '\0')) != NULL)
    {
        if (++count == num)
        {
            ...
            result[end - start] = '\0';
            if(result[0] == 13 && result[1] == 10){ //  13 e 10 eh o ASCII do \n \r
                return "-1";
            }
            return result;
        }
...
int comparaCampo(Registro *registro, char *nomeCampo, char *valorCampo, int valorCampo2) {
    if (strcmp(nomeCampo, "nomeTecnologiaOrigem") == 0) {
        return strncmp(registro->nomeTecnologiaOrigem.string, valorCampo, strlen(valorCampo)) == 0;
    } else if (strcmp(nomeCampo, "grupo") == 0) {
        return registro->grupo == valorCampo2;
        
    } else if (strcmp(nomeCampo, "popularidade") == 0) {
    
        return registro->popularidade == valorCampo2;
        
    } else if (strcmp(nomeCampo, "nomeTecnologiaDestino") == 0) {
        return strncmp(registro->nomeTecnologiaDestino.string, valorCampo, strlen(valorCampo)) == 0;
        
    } else if (strcmp(nomeCampo, "peso") == 0) {
        return (registro->peso == valorCampo2);
    }
...
void recuperaDadosBuscados(char *arquivoEntrada, char* arquivoEntradaIndice, int n) {
    ...
    int novoN = n; // serve para intercalar esta funcao e a da funcionalidade 6
    // char line[n][1024];
    ...
    for (int i = 0; i < n; i++) {
        ...
        if(!strcmp(nomeDoCampo[i], "popularidade") || !strcmp(nomeDoCampo[i], "grupo") || !strcmp(nomeDoCampo[i], "peso") ){
            scanf("%d", &valorDoCampo2[i]);
        } else{
            scan_quote_string(valorDoCampo[i]);
        }
        if ((strcmp(nomeDoCampo[i], "nomeTecnologiaOrigemDestino") == 0)){
            //recuperaDadosComIndiceArvoreB(arquivoEntrada, arquivoEntradaIndice,novoN);
            printf("arrumar isso daqui pra n precisar da arvoreB\n");
            return;
        }
        else{
           int regInexistente = procuraDados(binario, nomeDoCampo[i], valorDoCampo[i], valorDoCampo2[i]);
           if(regInexistente == -1){
            continue;
           }
           novoN--;
 ...
 int RRN = 0;
            ...
            while (RRN < qtdRegistros) {
                ...
                if (registro->removido == '0') {
                        if (comparaCampo(registro, nomeDoCampo, valorDoCampo,valorCampo)) {
...
void validarArquivo(FILE* binario){
    ...
    fread(&(arquivoValido), sizeof(char), 1, binario);
    if(arquivoValido == '0' || binario == NULL){
        printf("Falha no processamento do arquivo. \n");
        exit(0);
    }
}
